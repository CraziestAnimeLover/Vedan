...
const AharMultiple = () => {
  ...
  const handleAddCard = async () => {
    try {
      if (editIndex === null) {
        ...
        const response = await fetch(`http://localhost:8000/api/organizations/${cards[editIndex]._id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleDeleteCard = async (index) => {
    try {
      const response = await fetch(`http://localhost:8000/api/organizations/${cards[index]._id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
