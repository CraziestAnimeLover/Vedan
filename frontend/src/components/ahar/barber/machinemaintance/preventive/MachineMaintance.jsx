...
const MachineMaintance = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/machine-maintenance",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setCards([...cards, formData]); // Add to local state
      } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/machine-maintenance/${editingId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
...
export default MachineMaintance;
