...
const Notice = () => {
  ...
  useEffect(() => {
    ...
    const fetchNotices = async () => {
      setLoading(true);
      try {
        const response = await axios.get("http://localhost:8000/api/gym/notices");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editIndex !== null) {
        await axios.put(
          `http://localhost:8000/api/gym/notices/${notices[editIndex]._id}`,
          formData
        );
        ...
        const response = await axios.post("http://localhost:8000/api/gym/notices", formData);
  ...
  const removeNotice = async (index) => {
    try {
      await axios.delete(`http://localhost:8000/api/gym/notices/${notices[index]._id}`);
