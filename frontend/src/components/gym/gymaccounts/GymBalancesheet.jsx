...
const GymBalancesheet = () => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [activeMenu, setActiveMenu] = useState(null);
  const [selectedSection, setSelectedSection] = useState("Balance Sheet");
  ...
  const calculateTotal = (data) =>
    data.reduce((sum, item) => sum + item.rupees, 0);
    ...
    const fetchBalanceSheet = async () => {
      ...
      try {
        const response = await fetch(
          "http://localhost:8000/balancesheet/latest",
          {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/balancesheet/update",
        {
          method: "PUT",
    ...
    <div className="w-full h-screen fixed left-0 top-0 bg-white/20 backdrop-blur-lg shadow-lg flex">
                  ...
                  {serviceItems[menu].map((item) => (
                    <li key={item.title}>
                      {item.component ? (
                        ...
                        <Link
                          to={item.url}
                          className="block text-blue-600 hover:text-blue-800 p-2"
                        >
                          {item.title}
                        </Link>
                      )}
      ...
      <div className="flex flex-col items-center w-full h-screen overflow-y-auto">
        <div className="flex flex-col items-center w-full sticky top-0 bg-white z-10 py-4">
          <span className="text-4xl md:text-7xl font-semibold px-2">
            {selectedSection}
          </span>
          <hr className="w-full border-gray-800 mt-20 mb-4" />
        </div>

        <div className="w-full md:w-3/4 p-4 mx-auto"></div>
        <BalanceTables
          assets={assets}
...
export default GymBalancesheet;
